...
export default function CashierIncomeTable({
  cashiers,
  totals,
  loading,
}: Props) {
  ...
  const exportToCSV = () => {
    ...
    const csvData = [
      ...
      ...sortedCashiers.map((cashier) =>
        [
          ...
          `"${cashier.phone}"`,
          cashier.status,
          cashier.dailyIncome.toFixed(2),
          cashier.weeklyIncome.toFixed(2),
          cashier.monthlyIncome.toFixed(2),
          cashier.totalIncome.toFixed(2),
          cashier.totalRevenue.toFixed(2),
          cashier.totalBetAmount.toFixed(2),
          cashier.totalRegisteredNumbers,
          cashier.reportCount,
        ].join(",")
      ),
      // Add totals row
    ...
    link.setAttribute(
      "download",
      `cashier-income-report-${new Date().toISOString().split("T")[0]}.csv`
    );
    ...
    return (
      <div className="rounded-md border overflow-x-auto">
        <div className="p-4 border-b flex justify-between items-center">
          <h3 className="text-lg font-semibold">Cashier Income Details</h3>
  ...
  return (
    <div className="rounded-md border overflow-x-auto">
      ...
      <Table>
        <TableHeader>
          <TableRow>
            <SortableHeader field="name">Cashier</SortableHeader>
            <TableHead>Phone</TableHead>
            <TableHead>Status</TableHead>
        ...
        <TableBody>
          {sortedCashiers.map((cashier) => (
            <TableRow key={cashier.id} className="hover:bg-muted/50">
              ...
              <TableCell className="font-mono">
                {cashier.totalRegisteredNumbers.toLocaleString()}
              </TableCell>
          ...
          <TableRow className="bg-gray-400 font-semibold">
            ...
            <TableCell>${totals.dailyIncome.toFixed(2)}</TableCell>
            <TableCell>${totals.weeklyIncome.toFixed(2)}</TableCell>
            <TableCell>${totals.monthlyIncome.toFixed(2)}</TableCell>
